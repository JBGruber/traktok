...
Authentication for `traktok` happens automatically the first time you run a function.
To TikTok, this will make it look as if `traktok` requests come from your ordinary browser.
`traktok` will save a cookie file in the location returned by `tools::R_user_dir("traktok", "config")`.
If you ever run into problems due to an expired cookie, you might want to delete the files in this folder to get a fresh start.
